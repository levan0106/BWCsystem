        ...
        displayValue: {
            get: function() {
                ...
                if (this.isInputActive) {
                    ...
                    returnValue = this.value == null || isNaN(this.value/1)? 0: this.value.toString()
                } else {
                    // User is not modifying now. Format display value for user interface
                    returnValue = isNaN(this.value/1)? 0 :(this.value/1).toFixed(this.digit).replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, "$1,")
                    //return value
                }
                // debugger;
                // console.log(returnValue)
                // // dont allow input negative value
                // if(!this.negative && returnValue < 0){
